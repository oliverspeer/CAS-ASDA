...
Ausser den 456 fehlenden Nutzerzahlen, die nach der Modellierung vorhergesagt werden, sind keine weiteren fehlenden Werte vorhanden
...
Es zeigt isch ein gewisser gewisser Trend, dass die Nutzerzahlen mit steigender Temperatur zunehmen.
...
sharing_na.p <- sharing_na.p |> 
  mutate(
    user = exp(p$fit),
    lower_PI = apply(beob, 1, quantile, probs = 0.025, type = 1),
    upper_PI = apply(beob, 1, quantile, probs = 0.975, type = 1)
)
...
# # Vorhersagen berechnen
# sharing.p <- sharing
# sharing.t$predicted_user_nb <- predict(fit.nb, newdata = sharing.t, type = "response")
# 
# 
# # Streudiagramm: Beobachtete vs. Vorhergesagte Werte
# ggplot(sharing.t, aes(x = predicted_user_nb, y = user)) +
#   geom_point(alpha = 0.3) +
#   geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
#   labs(x = "Vorhergesagte Werte nb", y = "Beobachtete Werte", title = "Vorhergesagte vs. Beobachtete Werte nb")

```

```{r}
# # Residuen berechnen
# sharing.t$residuals.nb <- residuals(fit.nb, type = "pearson")
# 
# # Residuenplot gegen die Vorhersagen
# ggplot(sharing.t, aes(x = predicted_user_nb, y = residuals.nb)) +
#   geom_point(alpha = 0.3) +
