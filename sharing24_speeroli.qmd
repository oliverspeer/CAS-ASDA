...
Ziel ist es, die Nutzerzahlen zu modellieren, unter Berücksichtigung von Tageszeiten, Wochentagen und Wetterbedingungen. Mit einem resultierenden Modell sollen Vorhersagen für Tage mit fehlenden Nutzerzahlen erstellt.
...
<!-- ```{r echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- df <- sharing |> summarise(across(everything(), ~sum(is.na(.)))) |> flextable() |> autofit() |> align(align = "center", part = "all") -->
<!-- df -->
<!-- ``` -->
...
Ein gewisser Trend zeigt, dass die Nutzerzahlen mit steigender Temperatur etwas zunehmen.
...
Die Temperatur ist wie zu erwarten deutlich abhängig von der Tageszeit (saisonaler Anteil im STL Plot oben). Die Temperatur zeigt zusätzlich einen Trend-Anteil, das Wetter. Und sicher auch eine harmonische Schwingung über den Tagesverlauf.
...
### Modell mit Interaktionen und Schwingungen der Tageszeiten, Wochentage und Tage {.justify}
...
Das Model `fit.1` erklärt `r round(1-s1$deviance/s1$null.deviance, 2)*100`% der Devianz. Die Devianz konnte im Vergleich zum einfachen Modell deutlich reduziert werden. Die als Schwingungen modellierten Interaktionen zwischen Temperatur und Tageszeiten, als auch zwischen Wochen- & Feiertage und Stunden haben einen signifikanten Einfluss auf die Nutzerzahlen.  
...
Das Modell `fit.nb` erklärt `r round(1-s.nb$deviance/s.nb$null.deviance,1)*100`% der Devianz der Nutzerzahlen. Die Devianz konnte im Vergleich zum Poisson Modell weiter reduziert werden. 
...
# Prognose mit negativer Binomial-Model (fit.nb) und 95% Konfidenzintervall
...
Das ergänzte Datenfile wurde als `sharing_speeroli.rds` gespeichert.
